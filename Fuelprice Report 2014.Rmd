---
title: "Tankstellen und Kraftstoffpreise 2014"
...
# Tankstellen
Für die Übersicht der in Deutschland aktiven Tankstellen, wurden die Daten sämtlicher Tankstellen ausgewertet, welche im Beobachtungszeitraum (01.07. bis 31.12.) mindestens eine Preisinformation übermittelt haben.
...
```{r, fuel price by county and month, fig.width=10, fig.height=15, fig.align="center", message=F, eval=T}
  ...
  map_price_month_county_pe5 <- tm_shape(shp = price_month_county_shp, name = "Monthly Super E5 Price") + 
      tm_polygons(paste0("pe5_", seq(as.Date(paste0(year, "-07-01")), by = "month", length.out = 6)),
                  style = "fixed",
                  ...
                  colorNA = "grey90") +
      tm_facets(nrow = 2) + 
      tm_layout(title = format(seq(ISOdate(2000,7,1), by = "month", length.out = 6), "%B"),
                title.size = 1.7,
                title.position = c("center","top"),
  ...
  legend_map_pe5 <- tm_shape(shp = price_month_county_shp) + 
    tm_fill(paste0("pe5_", year, "-07-01"),
            style = "fixed",
            palette = "Blues", contrast = c(0.3, 1.0),
            breaks = breaks_price_month_county$pe5,
  ...
  map_price_month_county_pe10 <- tm_shape(shp = price_month_county_shp, name = "Monthly Super E10 Price") + 
      tm_polygons(paste0("pe10_", seq(as.Date(paste0(year, "-07-01")), by = "month", length.out = 6)),
                  style = "fixed",
  ...
  legend_map_pe10 <- tm_shape(shp = price_month_county_shp) + 
    tm_fill(paste0("pe10_", year, "-07-01"),
            style = "fixed",
            palette = "Oranges", contrast = c(0.3, 1.0),
            breaks = breaks_price_month_county$pe10,
  ...
  map_price_month_county_pdi <- tm_shape(shp = price_month_county_shp, name = "Monthly Diesel Price") + 
      tm_polygons(paste0("pdi_", seq(as.Date(paste0(year, "-07-01")), by = "month", length.out = 6)),
                  style = "fixed",
  ...
  legend_map_pdi <- tm_shape(shp = price_month_county_shp) + 
    tm_fill(paste0("pdi_", year, "-07-01"),
            style = "fixed",
            palette = "Greens", contrast = c(0.3, 1.0),
            breaks = breaks_price_month_county$pdi,
...
### Super E5
```{r, fuel price by county and month Super E5, fig.width=10, fig.height=7.5, fig.align="center", message=F, eval=T}
...
### Super E10
```{r, fuel price by county and month Super E10, fig.width=10, fig.height=7.5, fig.align="center", message=F, eval=T}
...
### Diesel
```{r, fuel price by county and month Diesel, fig.width=10, fig.height=7.5, fig.align="center", message=F, eval=T}
...
```{r, price development daily, fig.width=10, fig.height=5, fig.align="center", message=F, eval=T}
  ...
  brent <- brent_in %>%
    ...
    arrange(Datum) %>%
    filter(Datum >= paste0(year, "-07-01")) %>%
...
```{r, query prices by categories, fig.width=10, fig.height=5, fig.align="center", message=F, eval=T}
  ...
  # query 'normal' stations price
  query_normal_price <- paste0("
                    SELECT 
                      time_bucket('1 day', time::TIMESTAMP) as day, 
                      round(avg(pe5),3) as e5,
                      round(avg(pe10),3) as e10,
  ...
  normal_price <- dbGetQuery(dbcon, query_normal_price) %>%
    as_tibble()
...
```{r, price cycles , fig.width=10, fig.height=5, fig.align="center", message=F, eval=T}
  ...
  price_minute <- dbGetQuery(dbcon, query_price_minute) %>%
    ...
    mutate(time = seq(ISOdatetime(year,7,1,0,0,0), by = "15 min", length.out = 96)) %>%
...
```{r, price cycles by month , fig.width=10, fig.height=5, fig.align="center", message=F, eval=T}
  ...
  price_minute_month <- dbGetQuery(dbcon, query_price_minute_month) %>%
    as_tibble() %>%
    # replace extracted HH:mm by datetime
    mutate(time = rep(seq(ISOdatetime(year,7,1,0,0,0), by = "15 min", length.out = 96), 6)) %>%
    select(-c(hour, minute)) %>%
    group_by(month) %>%
  ...
  ggplot(price_minute_month, aes(x = time)) + 
    ...
    scale_colour_viridis_d(name = "",
                           labels = format(seq(ISOdate(2000,7,1), by = "month", length.out = 6), "%B"))
```
...
```{r, price cycles by month and weekday , fig.width=10, fig.height=5, fig.align="center", message=F, eval=T}
  ...
  query_price_minute_monthday <- paste0("
                    ...
                    FROM (
                    	SELECT
                    	  ...
                    	  locf(round(avg(pe5), 3)) AS pe5_avg
                      FROM price
                      WHERE (time BETWEEN '", year, "-07-01 00:00:01' AND '", year, "-12-31 23:59:59') AND
                    		    (pe5 IS NOT NULL)
                      GROUP BY dbtime
  ...
  price_minute_monthday <- price_minute_monthday_db %>%
    ...
    mutate(time = rep(seq(ISOdatetime(year,7,1,0,0,0), by = "1 hour", length.out = 24*7), 6)) %>%
    select(-c(day, hour)) %>%
    # remove monthly mean
    group_by(month) %>%
    mutate(avg = avg - mean(avg))
